use super::{
    GroupError, GroupLeaveValidationError, MlsGroup, intents::QueueIntent,
    validated_commit::LibXMTPVersion,
};
use crate::{
    context::XmtpSharedContext,
    groups::{intents::ReaddInstallationsIntentData, send_message_opts::SendMessageOpts},
};
use std::collections::{HashMap, HashSet};
use xmtp_common::{Event, log_event};
use xmtp_configuration::MAX_GROUP_SIZE;
use xmtp_content_types::{
    ContentCodec, encoded_content_to_bytes, leave_request::LeaveRequestCodec,
};
use xmtp_db::{
    group::ConversationType,
    pending_remove::QueryPendingRemove,
    prelude::{QueryGroup, QueryGroupVersion},
};
use xmtp_id::{AsIdRef, InboxIdRef, associations::Identifier};
    ...
    Context: XmtpSharedContext,
{
    ...
    ) -> Result<super::intents::UpdateGroupMembershipResult, GroupError> {
    ...
    pub async fn add_members<S: AsIdRef>(
        ...
        let ok_result = Ok(super::intents::UpdateGroupMembershipResult::from(
            intent_data.clone(),
        ));
    ...
    pub(crate) async fn ensure_not_paused(&self) -> Result<(), GroupError> {
